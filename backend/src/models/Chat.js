...
exports.getUserChats = async (userId) => {
  const res = await pool.query("SELECT * FROM chats WHERE user_id = $1", [
    userId,
  ]);
  return res.rows;
};

exports.getPetOwnerChats = async (petOwnerId) => {
  const res = await pool.query("SELECT * FROM chats WHERE pet_owner_id = $1", [
    petOwnerId,
  ]);
  return res.rows;
};
